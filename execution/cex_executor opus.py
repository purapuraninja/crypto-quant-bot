...
class CEXExecutor(BaseExecutor):
    ...
    def _calc_roi_sl(action: str, entry: float, leverage: float,
                     price_dp: int = 6,
                     max_price_move_pct: float = 6.0) -> float:
        """
        Calculate SL price at -100% ROI (near-liquidation), but capped at
        max_price_move_pct from entry to prevent catastrophic losses.

        Problem without cap: 5x leverage → SL allows -20% price move.
        With cap=6%: SL is placed at max 6% from entry regardless of leverage.

        Formula:
          LONG  → SL = entry × (1 − 1/leverage) × buffer, capped at entry × (1 - max_pct)
          SHORT → SL = entry × (1 + 1/leverage) / buffer, capped at entry × (1 + max_pct)
        """
        buffer = 0.995  # 0.5% buffer away from exact liquidation
        max_f  = max_price_move_pct / 100.0

        if action == "LONG":
            sl_roi = entry * (1.0 - 1.0 / leverage) * buffer
